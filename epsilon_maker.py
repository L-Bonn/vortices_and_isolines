...
def writevortpath(path, folder, overwrite=False, scalarjson=False):
    ...
    if scalarjson:
        full = 'full'
    destpath = f"/lustre/astro/rsx187/isolinescalingdata/{full}epsilondata/{folder}/{name}"
    ...
    for i in frameis:
        ...
        if not overwrite:
            if os.path.isfile(f'{destpath}/epsilon/frame{i}.npy'):
                continue
        try:
        ...
        if scalarjson:
            with open(f'{destpath}/epsilon/frame{i}.json', 'w') as f:
                json.dump(epsilon.tolist(), f)
        else:
            epsilonbin = epsilon>0
            np.save(f'{destpath}/epsilon/frame{i}.npy', epsilonbin)
...
paths = [p for p in paths if "params_0" in p]
